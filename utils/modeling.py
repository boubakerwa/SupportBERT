...
#         extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        ...
        P_att = P_att.to(dtype=torch.float32) # fp16 compatibility #changed!
        P_att = (1.0 - P_att) * -10000.0

        Q_att = Q_att.unsqueeze(1).unsqueeze(2)
        Q_att = Q_att.to(dtype=torch.float32) # fp16 compatibility #changed!
        Q_att = (1.0 - Q_att) * -10000.0

        A_att = A_att.unsqueeze(1).unsqueeze(2)
        A_att = A_att.to(dtype=torch.float32) # fp16 compatibility #changed!
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config, num_choices):
        ...
        self.flat_token_type_ids = None


    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, inputs_embeds=None, return_logits=False):
        flat_input_ids = input_ids.view(-1, input_ids.size(-1))
        flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1)) if token_type_ids is not None else None
        flat_attention_mask = attention_mask.view(-1, attention_mask.size(-1)) if attention_mask is not None else None
        self.flat_token_type_ids = flat_token_type_ids
        #_, pooled_output = self.bert(flat_input_ids, flat_token_type_ids, flat_attention_mask, output_all_encoded_layers=False)

        extended_attention_mask,c_attention_mask,q_attention_mask,a_attention_mask,sequence_output = self.bert(flat_input_ids, flat_token_type_ids, flat_attention_mask, output_all_encoded_layers=False, inputs_embeds=inputs_embeds)
