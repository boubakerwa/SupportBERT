...
class MTDNNModel(object):
    ...
    def _model_init(self, model, state_dict=None, num_train_step=-1):
    ...
    def _get_param_groups(self):
        param_optimizer = [n for n in list(self.network.named_parameters()) if 'pooler' not in n[0]]
    ...
    def _setup_lossmap(self, config):
        ...
        lc = LOSS_REGISTRY[cs](name='Loss func of task {}: {}'.format(0, cs)) 
    ...
    def _setup_adv_lossmap(self, config):
        ...
        if config.get('adv_train', False):
            cs = config['adv_loss']
            assert cs is not None
            lc = LOSS_REGISTRY[cs](name='Adv Loss func of task {}: {}'.format(0, cs))
